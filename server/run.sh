...
echo "  [Delete chat image]"
docker image rm apac_chat_img -f
...
echo " [Building image 'apac_chat_img' and put files inside]"
docker build ./chat -t apac_chat_img
docker create --name temp_container_chat apac_chat_img
docker cp chat temp_container_chat:/code/
docker cp utils temp_container_chat:/code/
docker commit temp_container_chat apac_chat_img
docker rm temp_container_chat -f
